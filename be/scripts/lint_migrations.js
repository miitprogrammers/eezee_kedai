...
function scan() {
  ...
  files.forEach((file) => {
    ...
    if (hasAdd) {
      ...
      if (!hasDescribe) {
        problems.push({ file, reason: 'uses addColumn without describeTable check' });
      }
    }
  });

  if (problems.length > 0) {
    console.error('Migration lint failed: found potential non-idempotent addColumn usages:\n');
