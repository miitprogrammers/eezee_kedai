...
exports.stockIn = async (req, res) => {
  ...
  if (!item_id || !cabang_id || jumlah == null || harga_beli == null) {
    return res
      ...
      .json({
        error:
          "Missing required fields: item_id, cabang_id, jumlah, harga_beli",
      });
  }
  if (
    isNaN(Number(jumlah)) ||
    isNaN(Number(harga_beli)) ||
    Number(jumlah) <= 0 ||
    Number(harga_beli) < 0
  ) {
    return res
      .status(422)
      .json({ error: "Invalid numeric values for jumlah or harga_beli" });
  }
  const maxAttempts = 3;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      appendActivity("Stock IN failed", {
        ...
        details: {
          how: "stockIn",
          plan: isValidationErr ? "fix request payload" : "inspect and retry",
        },
      });
      const status = isValidationErr ? 422 : 500;
      const message = isValidationErr
        ? err.errors
